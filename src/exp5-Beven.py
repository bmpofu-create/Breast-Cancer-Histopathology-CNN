...
# Dictionaries to persist trained references for the real-time inference test
trained_references = {}
...
for mag in MAGNIFICATION_TIERS:
        ...
        if mag.lower() in path_parts:
            if "benign" in path_parts or "malignant" in path_parts:
                ...
                try:
                    ...
                    sub_label = path_parts[anchor_idx + 1]  # Captures adenosis, fibroadenoma, etc.
    ...
    # Keep track of training references for deployment testing later
...
# -------------------------------------------------------------------------
# 6. TESTING PIPELINE FOR NEW IMAGES (ADDED)
# -------------------------------------------------------------------------
def predict_image(image_path, magnification_tier):
    """
    Predicts both the core class (Binary) and the subtype of a given test image.
    """
    ...
    # Step 1: Extract embedding for the test image
    try:
    ...
    # Step 2: Grab the matching training references for this magnification
    ...
    # Step 3: Run Cosine Similarity against the training block (1-NN)
    similarity = np.dot(test_embedding, X_train.T)  # Shape: (1, num_train_samples)
